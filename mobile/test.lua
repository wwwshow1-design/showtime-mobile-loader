...
State.VehicleRefreshButton = Refresh
...
State.VehicleCloseButton = Close

-- 모바일 차량 선택 창 전용 뒤로가기입니다. PC 배치는 그대로 유지합니다.
State.VehicleBackButton = Instance.new("TextButton")
State.VehicleBackButton.Size = UDim2.fromOffset(1, 1)
State.VehicleBackButton.Position = UDim2.fromOffset(0, 0)
State.VehicleBackButton.BackgroundColor3 = Color3.fromRGB(55, 62, 78)
State.VehicleBackButton.BorderSizePixel = 0
State.VehicleBackButton.Text = "←"
State.VehicleBackButton.TextColor3 = Color3.new(1, 1, 1)
State.VehicleBackButton.TextSize = 22
State.VehicleBackButton.Font = Enum.Font.GothamBold
State.VehicleBackButton.Visible = false
State.VehicleBackButton.Parent = Main
State.VehicleBackCorner = Instance.new("UICorner")
State.VehicleBackCorner.CornerRadius = UDim.new(0, 6)
State.VehicleBackCorner.Parent = State.VehicleBackButton
...
State.CloseVehiclePickerView = function()
    Main.Visible = false
    if type(SetAttackPanelVisible) == "function" then
        SetAttackPanelVisible(true)
    end
end
Close.MouseButton1Click:Connect(State.CloseVehiclePickerView)
State.VehicleBackButton.MouseButton1Click:Connect(State.CloseVehiclePickerView)
...
if MobileUI.Enabled then
    ...
    -- 자동팜의 기존 X 동작은 그대로 두고 홈/뒤로/3D+UI를 독립 버튼으로 추가합니다.
    Close.Text = "X"
    Close.Size = UDim2.fromOffset(44, 40)
    Close.Position = UDim2.new(1, -52, 0, 6)
    Close.TextSize = 18

    MobileUI.MakeTopButton = function(Text, Position, Width, Color)
        local Button = Instance.new("TextButton")
        Button.Size = UDim2.fromOffset(Width or 44, 40)
        Button.Position = Position
        Button.BackgroundColor3 = Color or Color3.fromRGB(55, 62, 78)
        Button.BorderSizePixel = 0
        Button.Text = Text
        Button.TextColor3 = Color3.new(1, 1, 1)
        Button.TextSize = Text == "3D+UI" and 12 or 22
        Button.Font = Enum.Font.GothamBold
        Button.Parent = Main
        local Corner = Instance.new("UICorner")
    ...
    MobileUI.FarmHomeButton = MobileUI.MakeTopButton("⌂", UDim2.new(1, -284, 0, 6), 52)
    MobileUI.FarmBackButton = MobileUI.MakeTopButton("←", UDim2.new(1, -228, 0, 6), 44)
    MobileUI.Farm3DButton = MobileUI.MakeTopButton("3D+UI", UDim2.new(1, -180, 0, 6), 76, Color3.fromRGB(62, 82, 118))
    MobileUI.FarmMini3DButton = MobileUI.MakeTopButton("3D+UI", UDim2.new(1, -180, 0, 6), 76, Color3.fromRGB(62, 82, 118))
    MobileUI.FarmMini3DButton.Visible = false

    MobileUI.FarmMinimizeButton = MobileUI.MakeTopButton("─", UDim2.new(1, -100, 0, 6), 44)

    Title.Size = UDim2.new(1, -310, 0, 40)
    ...
    MobileUI.SetFarmMinimized = function(Minimized)
        ...
        if MobileUI.FarmMinimized then
            ...
            MobileUI.Farm3DButton.Visible = false
            MobileUI.FarmMini3DButton.Visible = true
            MobileUI.FarmMinimizeButton.Text = "□"
        else
            MobileUI.FarmMiniVehicle.Visible = false
            MobileUI.FarmMiniStatus.Visible = false
            MobileUI.Farm3DButton.Visible = true
            MobileUI.FarmMini3DButton.Visible = false
            MobileUI.FarmMinimizeButton.Text = "─"
            TabBar.Visible = true
    ...
    MobileUI.HideFarmForIntegratedUI = function(Home)
        Gui.Enabled = false
        if Root and Root.Parent then Root.Visible = false end
        local Bridge = getgenv and getgenv().ShowTimeIntegratedBridge
        if not Bridge then return end
        if Home and type(Bridge.ReturnToHome) == "function" then
            Bridge.ReturnToHome()
        elseif type(Bridge.ReturnToMain) == "function" then
            Bridge.ReturnToMain()
        end
    end
    MobileUI.FarmHomeButton.MouseButton1Click:Connect(function()
        MobileUI.HideFarmForIntegratedUI(true)
    end)
    MobileUI.FarmBackButton.MouseButton1Click:Connect(function()
        MobileUI.HideFarmForIntegratedUI(false)
    end)
    MobileUI.Toggle3DAndUI = function()
        local Bridge = getgenv and getgenv().ShowTimeIntegratedBridge
        if Bridge and type(Bridge.Toggle3DAndUI) == "function" then
            Bridge.Toggle3DAndUI()
        end
    end
    MobileUI.Farm3DButton.MouseButton1Click:Connect(MobileUI.Toggle3DAndUI)
    MobileUI.FarmMini3DButton.MouseButton1Click:Connect(MobileUI.Toggle3DAndUI)
    ...
    MobileUI.ApplyViewport = function()
        ...
        if State.VehicleBackButton then
            State.VehicleBackButton.Visible = true
            State.VehicleBackButton.Size = UDim2.fromOffset(44, 40)
            State.VehicleBackButton.Position = UDim2.fromOffset(10, 6)
            State.VehicleTitle.Position = UDim2.fromOffset(64, 5)
            State.VehicleTitle.Size = UDim2.new(1, -260, 0, 42)
            State.VehicleRefreshButton.Size = UDim2.fromOffset(72, 40)
            State.VehicleRefreshButton.Position = UDim2.new(1, -132, 0, 6)
            State.VehicleCloseButton.Size = UDim2.fromOffset(44, 40)
            State.VehicleCloseButton.Position = UDim2.new(1, -52, 0, 6)
        end
        if DetailClose then
    ...
    task.spawn(function()
        while Gui and Gui.Parent do
            ...
            local Bridge = getgenv and getgenv().ShowTimeIntegratedBridge
            if Bridge and type(Bridge.Get3DAndUIState) == "function" then
                MobileUI.Farm3DButton.Text = Bridge.Get3DAndUIState() and "3D+UI 끄기" or "3D+UI 켜기"
                MobileUI.Farm3DButton.TextSize = 10
                MobileUI.FarmMini3DButton.Text = MobileUI.Farm3DButton.Text
                MobileUI.FarmMini3DButton.TextSize = 10
            end
            task.wait(0.5)
        end
    end)
...
IntegratedBridge.ReturnToHome = function()
	UI.Main.Visible = true
	UI.SetCompactMode(false)
	if type(UI.ShowV11Page) == "function" then
		UI.ShowV11Page("대시보드")
	end
end

IntegratedBridge.Get3DAndUIState = function()
	return RuntimeState.RenderingEnabled and RuntimeState.GameUIVisible
end

IntegratedBridge.Toggle3DAndUI = function()
	local TargetState = not IntegratedBridge.Get3DAndUIState()
...
UI.IntegratedMainButton.MouseButton1Click:Connect(function()
	IntegratedBridge.Toggle3DAndUI()
end)
